...
The second click is also when the **green dot and checkered flag** appear on the route's two ends —
the same pair the explore panel puts on a segment, answering the same question, since a line on a
map has no visible direction and by twenty miles in the two ends of a route are nowhere near each
other. Not before: which end is the start is exactly what one segment has not decided, and marking
one anyway would be the same lie the gain range exists to avoid.
