...
function grid(): { coord: Coord; item: string }[] {
  ...
  for (let row = 0; row < 10; row++) {
    for (let col = 0; col < 10; col++) {
      entries.push({
        coord: [-122.33 + col * 0.001, 47.68 + row * 0.001],
        item: `${row},${col}`,
      });
    }
  }
...
describe("SpatialIndex", () => {
  it("finds a point sitting exactly on the query", () => {
    ...
    const hits = index.within([-122.33, 47.68], 1);
  ...
  it("returns hits nearest first", () => {
    const index = new SpatialIndex(grid(), 200);
    const hits = index.within([-122.3295, 47.6805], 200);
  ...
  it("agrees with a brute-force scan", () => {
    ...
    const query: Coord = [-122.3262, 47.6833];
    const radius = 250;
  ...
  it("excludes points beyond the radius", () => {
    ...
    const hits = index.within([-122.33, 47.68], 120);
  ...
  it("refuses a radius wider than a cell, which the 3×3 scan cannot cover", () => {
    ...
    expect(() => index.within([-122.33, 47.68], 400)).toThrow(
  ...
  it("returns nothing for an empty index or a far-away query", () => {
    expect(
      new SpatialIndex<string>([], 100).within([-122.28, 47.63], 50),
    ).toEqual([]);
    const index = new SpatialIndex(grid(), 200);
  ...
  it("gives the closest point, or null when nothing is in range", () => {
    const index = new SpatialIndex(grid(), 200);
    expect(index.nearest([-122.3291, 47.6801], 200)?.item).toBe("0,1");
    expect(index.nearest([-122.33, 47.68], 0.001)?.item).toBe("0,0");
